...
void Langevin_position_dicrete_Fokker();
...
{
    ofstream posFile;
    posFile.open("/Users/Naanaa/Desktop/Control/thermalNoiseInput_outputPosition_1sec.txt");
    double time=0.0;
    double position=0.0;
    double inputNoise;
    ...
    for( int n=0;n<N-1;n++)
    {
        ...
        posFile<<time<<","<<inputNoise<<","<<position<<endl;
        position=position+D1*dts*position+inputNoise;
        time=time+dts;
        if(n%100000==0)
            cout<<time<<endl;
    }
...
{
    ofstream outWLC;
    outWLC.open("/Users/Naanaa/Documents/research/Trap_WLC_simulation/pullingDNA_20111027/pull_100speed_100kHz_elastin.txt");
    ...
    do
    {
        WLC=(kb*temp/0.36e-9)*(1/(4*(pow(1-(extension/166e-9),2)))+(extension/166e-9)-0.25);
...
//    double time_noWLC[N];
    ...
    Langevin_position_dicrete_Fokker(); //calculates position of the trapped bead when there is no WLC force
